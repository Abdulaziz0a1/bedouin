...
import { COHOSTS, SERVICE_LABELS } from "@/lib/data/cohost";
import type { CoHost, CoHostService } from "@/lib/types/cohost";
...
const ALL_REGIONS = [...new Set(COHOSTS.map((c) => c.region))].sort();
...
export default function CoHostBrowsePage() {
  const [cohosts,          setCohosts]        = useState<CoHost[]>(COHOSTS);
  ...
  const selectedCohost = cohosts.find((c) => c.id === selectedId) ?? null;
  ...
  const filtered = useMemo(() => {
    ...
    if (serviceFilters.length > 0) {
      result = result.filter((c) =>
        serviceFilters.every((s) => c.services.includes(s))
      );
    }
  ...
  }, [cohosts, search, regionFilter, serviceFilters]);
  ...
  const handleInviteConfirm = async (
    ...
    setInviting(true);
    // Firestore: addDoc(collection(db,"collaborations"), { hostId, coHostId, listingId, message, status:"pending", sentAt: serverTimestamp() })
    await new Promise<void>((r) => setTimeout(r, 900));
    setCohosts((prev) =>
      prev.map((c) =>
        c.id === selectedCohost.id ? { ...c, status: "invited" as const } : c
      )
    );
    setInviting(false);
  ...
  const availableCount = cohosts.filter((c) => c.status === "available").length;
    ...
    <div className="flex flex-col" style={{ height: "100dvh" }}>
      ...
      <header className="bg-white border-b border-[#e8dfd4] sticky top-0 z-40 shrink-0">
        <div className="max-w-[1440px] mx-auto px-5 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <a
              href="/dashboard"
              className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold text-[#64707d] hover:bg-[#f4f6f8] transition-colors"
            >
      ...
      <div className="bg-[#1a0e02] shrink-0 px-5 py-4">
        <div className="max-w-[1440px] mx-auto flex items-center justify-between gap-4 flex-wrap">
          ...
          <a
            href="/dashboard?tab=cohosts"
            className="flex items-center gap-2 px-4 py-2 bg-[#8b5e38] text-white text-xs font-semibold rounded-xl hover:bg-[#7a5030] transition-colors"
          >
            Manage my co-hosts
          </a>
        </div>
              ...
              <button
                ...
                className={`text-[10px] font-bold px-2.5 py-1 rounded-full border transition-all ${
                  !regionFilter
                    ...
                    : "text-[#64707d] border-[#e8dfd4] hover:border-[#8b5e38]"
                }`}
              >
                All regions
              </button>
            ...
            <div className="flex gap-1.5 overflow-x-auto pb-0.5" style={{ scrollbarWidth: "none" }}>
              {ALL_SERVICES.map((s) => (
                <button
                  ...
                  className={`text-[10px] font-bold px-2.5 py-1 rounded-full border shrink-0 transition-all ${
                    serviceFilters.includes(s)
                      ? "bg-[#c49a4f] text-white border-[#c49a4f]"
                      : "text-[#64707d] border-[#e8dfd4] hover:border-[#c49a4f]"
                  }`}
                >
                  {SERVICE_LABELS[s]}
                </button>
