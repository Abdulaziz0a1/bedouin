...
import { sortByScore } from "@/lib/utils/discovery-ranking";
import { localizeText } from "@/lib/types/discovery";

export default function TopSights() {
  const { t, lang } = useLanguage();

  const sights = sortByScore(destinationRegions.filter((r) => r.visible));
  const [featured, ...stacked] = sights;
      ...
      <div className="max-w-[1232px] mx-auto px-6 lg:px-0 flex flex-col gap-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <FadeInSection direction="left" delay={60}>
            ...
            >
              <Image src={featured.image} alt={localizeText(featured.name, lang)} fill
              ...
              <div className="absolute top-5 left-5 flex items-center gap-2">
                ...
                <span className="text-[#c49a4f] text-[10px] font-bold uppercase tracking-widest">
                  {localizeText(featured.region, lang)}
                </span>
              </div>
              ...
              <div className="absolute bottom-0 left-0 p-6 text-white">
                <h3 className="font-display font-extrabold text-4xl mb-2 tracking-tight">
                  {localizeText(featured.name, lang)}
                </h3>
                <p className="text-white/78 text-sm leading-relaxed max-w-xs">
                  {localizeText(featured.description, lang)}
                </p>
                <div className="flex items-center gap-1.5 mt-4 opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-1 group-hover:translate-y-0">
                  <span className="text-[#c49a4f] text-xs font-bold uppercase tracking-[0.14em]">{t("nav.explore")}</span>
          ...
          <div className="flex flex-col gap-5">
            {stacked.map((s, i) => (
              <FadeInSection key={s.id} direction="right" delay={80 + i * 100}>
                <Link href={s.href}
                ...
                >
                  <Image src={s.image} alt={localizeText(s.name, lang)} fill
                    className="object-cover group-hover:scale-[1.06] transition-transform duration-700"
                    style={{ transitionTimingFunction: "cubic-bezier(0.16,1,0.3,1)" }}
                    sizes="(max-width:768px) 100vw, 50vw" />
                  <div className="absolute inset-0"
                  ...
                  <div className="absolute top-4 left-4 flex items-center gap-2">
                    ...
                    <span className="text-[#c49a4f] text-[9px] font-bold uppercase tracking-widest">
                      {localizeText(s.region, lang)}
                    </span>
                  </div>
                  <div className="absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
                    style={{ boxShadow: "inset 0 0 0 1.5px rgba(196,154,79,0.40)" }}
                  ...
                  <div className="absolute bottom-0 left-0 p-5 text-white">
                    <h3 className="font-display font-bold text-2xl mb-1 tracking-tight">
                      {localizeText(s.name, lang)}
                    </h3>
                    <p className="text-white/72 text-xs leading-relaxed line-clamp-2">
                      {localizeText(s.description, lang)}
                    </p>
                  </div>
                </Link>
              </FadeInSection>
